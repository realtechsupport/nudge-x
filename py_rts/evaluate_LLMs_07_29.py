...
class CaptionEvaluator:
    ...
    def evaluate(self, caption: str, model: str, weights: dict = None, threshold: float = 2.5) -> dict:
        """
        ...
        # Adjust evaluation logic if scores are zero
        decision = self._calculate_decision(evaluation_result, weights, threshold)

        # Determine the decision text
    ...
    def _calculate_decision(self, evaluation_result: dict, weights: dict = None, threshold: float = 2.5) -> int:
        ...
        # Adjust evaluation to handle zero scores (don't arbitrarily fill them)
        adjusted_scores = {key: (evaluation_result.get(key, 0) if evaluation_result.get(key, 0) > 0 else 0) for key in weights.keys()}

        # Calculate weighted score using adjusted scores
        weighted_score = sum(adjusted_scores[key] * weights.get(key, 0) for key in weights)

        # Return 1 if weighted score meets or exceeds threshold, else return 0
    ...
    def _create_judge_prompt(self, caption: str) -> str:
        ...
        * **6. Conciseness of Expression (1-5):**
        ...
        3.  Then calculate the "Overall_Content_Score" as the average of the first five criteria (Environmental_Impact, Accuracy_Plausibility, Specificity_Terminology, Processes_Patterns, Adherence_to_Constraints). The "Conciseness" score is separate.
    ...
    def _parse_llm_response(self, json_string: str) -> dict:
        ...
        if parsed_json:
            ...
            return {
                "Environmental_Impact": float(parsed_json.get("Environmental_Impact", 0)),
                "Accuracy_Plausibility": float(parsed_json.get("Accuracy_Plausibility", 0)),
                "Specificity_Terminology": float(parsed_json.get("Specificity_Terminology", 0)),
                "Processes_Patterns": float(parsed_json.get("Processes_Patterns", 0)),
